%% Exp8_1: WCI-MC FOV Simulation (Strong Turbulence Only)
% 实验目的: 分析接收视场角 (FOV) 对链路衰减的影响 (包含强湍流影响的无偏估计)
clc; clear; close all;
...
Rx_Aperture = 0.01; Rx_Area = pi * (Rx_Aperture / 2)^2;
...
PL_Cell_WCI_Strong_FOV = cell(1, num_W);

fprintf('--- 运行 Exp8_1: WCI-MC FOV分析 (Strong Turbulence) ---\n');
...
for w_idx = 1:num_W
    ...
    PL_Cell_WCI_Strong_FOV{w_idx} = PL_matrix;
end
save('data_exp8_WCIMC_Strong_FOV.mat', 'dist_cell', 'PL_Cell_WCI_Strong_FOV', 'water_types', 'fov_angles_deg');

% ================= 辅助函数区域 =================
% (保持不变，此处包含 ray_march_flat_scalar, calc_haltrin_params, pdf_Empirical, rotate_direction_fast, 
% get_OTOPS_spectrum_handle, gen_screen_from_spectrum, calc_turb_coherence_params 等辅助函数，请直接复用前一代码中的内容，为保证版面整洁不再重复列出所有辅助函数)
