function [cdf_scatter,petzold_angle_rad] = generate_scatter1(func_type,water_cond,g)
...
%func_type = 'calc'      % 'measured' (测量值) 或 'calc' (计算值)
...
if strcmp(func_type,'measured')
    ...
    elseif strcmp(water_cond,'maalox_alan')
        ...
        load maalox_alan
        %load maalox_alan % 这些是直接测量值，并且没有对小角度进行外插
        
%         petzold_angle = [maalox_angle(1:99); [4.30:0.05:180]'];   %仅在不外插值时使用
...
elseif strcmp(func_type,'calc')
    ...
    if strcmp(water_cond,'hg')
        ...
        b = c-a;
        albedo = (c-a)/c;   % 水体反照率是散射系数/衰减系数 (b/c，无量纲)
