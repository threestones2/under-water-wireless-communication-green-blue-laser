...
% 计算TEOS-10相关参数
[c_p, sigma_T, mu, rho] = calculate_teos10_params(T_avg, S_avg); % 简化版
...
% 计算温度-盐度相关参数
function [chi_S, chi_TS, d_r, omega] = calculate_TS_params(chi_T, H)
    % 计算温度-盐度相关参数
    alpha_c = 2e-4;  % 热膨胀系数    
    beta_c = 7.6e-4; % 盐收缩系数
...
function gamma = calculate_turbulence_attenuation(lambda, epsilon, chi_T, T_avg, S_avg, H, eta)
    ...
    % 计算温度-盐度相关参数
    [chi_S, chi_TS, ~, ~] = calculate_TS_params(chi_T, H);
    
    % 计算温度、盐度和协方差谱
    kappa = linspace(0, 2*k, 1000);
    Phi_T = calculate_spectrum(kappa, epsilon, chi_T, eta, T_avg, S_avg, 'T');
    Phi_S = calculate_spectrum(kappa, epsilon, chi_S, eta, T_avg, S_avg, 'S');
