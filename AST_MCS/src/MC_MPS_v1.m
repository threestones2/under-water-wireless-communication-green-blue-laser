...
% 发射端指向偏差 (Tx Pointing Error)
theta_tx_error = 0;      % 发射端偏离主链路方向的极角 (rad)，例如 2 mrad
phi_tx_error = 0;           % 发射端偏离的方位角 (rad)

% 接收端指向偏差 (Rx Pointing Error)
theta_rx_error = 0;      % 接收端法线偏离对准方向的极角 (rad)
phi_rx_error = pi;          % 接收端偏离的方位角 (rad)

% 利用现有的 rotate_direction 函数生成偏转后的真实光轴向量
...
% --- 自动计算水体固有相函数的等效发散角 (1/e^2 宽度法) ---
% 1. 获取正向极小角度(近似为0)的相函数峰值
theta_peak = 1e-6; % 避免除零奇异点
p_peak = pdf_Empirical(cos(theta_peak), param);
target_p = p_peak * exp(-2); % 目标值为峰值的 1/e^2

% 2. 定义目标函数，寻找使得 p(theta) - target_p = 0 的 theta
% 使用 fzero 在小角度区间 (例如 1e-6 到 0.1 rad) 内寻根
obj_func = @(th) pdf_Empirical(cos(th), param) - target_p;

try
    opts = optimset('Display', 'off');
    % Petzold 前向散射极强，根通常在 0.001 ~ 0.05 rad 之间
    theta_water_calc = fzero(obj_func, [1e-6, 0.1], opts); 
catch
    % 如果求根失败（极罕见），使用加权均方根(RMS)作为后备计算方案
    th_test = linspace(1e-6, pi/4, 2000); 
    p_test = zeros(size(th_test));
    for idx = 1:length(th_test)
        p_test(idx) = pdf_Empirical(cos(th_test(idx)), param);
    end
    mean_sq_theta = 2 * pi * trapz(th_test, (th_test.^2) .* p_test .* sin(th_test));
    theta_water_calc = sqrt(mean_sq_theta);
end

param.theta_water = theta_water_calc;
fprintf('根据 Petzold 经验参数计算得到的等效水体发散角 theta_water = %.4f rad\n', param.theta_water);

% --- 湍流与相位屏 ---设置弱湍流参数
% --- 修改为论文 Table 1 的弱湍流参数 ---
T_avg = 20;             % 平均温度 20°C 
...
for s_idx = 1:2
    ...
    for p = 1:N_packets
        ...
        pos = Tx_Pos; dir = mu_T;%直射路径已经通过光束拓展解析式考虑了光子初始位置偏离光轴中心的问题了，必须直接模拟光子从光轴出射
        ...
        if plane_hit
            ...
            % 4. 计算接收能量 (基于 Marcum Q-函数的严格二维高斯曲面积分)
            cos_rx_tilt = abs(dot(dir_end, Rx_Normal));
            
            % 视角截断判定：若直射光束到达角度超出了 Rx_FOV，则强制设为未命中
            if acos(cos_rx_tilt) > Rx_FOV / 2
                received_fraction = 0;
            else
                % 等效投影孔径半径 (考虑法线倾斜导致的有效接收面积衰减)
                r_eff = Rx_Aperture * sqrt(cos_rx_tilt);
                
                % Marcum Q-函数的参数映射
                a_param = 2 * r_wander / W_spot; % 非中心参量 (偏轴惩罚)
                b_param = 2 * r_eff / W_spot;    % 积分上限参量 (孔径截断)
                
                % 严格的空间积分接收比例，该值在数学上绝对被约束在 [0, 1] 之间
            ...
            % 5. 最终能量权重 (直接相乘即可，去除了人为的 min(1, ...) 强制截断)
            ...
            if is_in_FOV_theta <= Rx_FOV/2
                ...
                if base_energy_pis > 1e-15 % 剔除极小权重以加速
                     if strcmp(scenario_name, 'With Turbulence')
                        ...
                        Huge_Aperture = 1e5;
                        [pos_end, ~, v_hit_flag, v_len] = ray_march_generic(pos, dir_to_rx, dist_to_rx + 1e-1, ...
                            Rx_Pos, Huge_Aperture, pi, Rx_Normal, true, ...
                            Screen_Chain, k_wave, x_axis, dx, N_grid, Tx_Pos, Link_Dir, delta_z_screen);
                        ...
                        if v_hit_flag
                            ...
                            % --- 替换后 ---
                            % 利用预计算的整段链路 rho0_Link 进行按比例动态缩放
                            % 物理依据：球面波 rho_0 正比于 L^(-3/5)
    ...
    % --- Step 2.3: 存储结果 ---
    h_time_norm = h_time / N_packets;      % 离散能量接收概率 (无量纲/光子份额)
    P_rx_total = sum(h_time_norm);         % 离散求和：等价于连续空间的能量积分

    results(s_idx).h_time_energy = h_time_norm;          % 保留离散分布用于后续时延计算
    results(s_idx).h_time_cir = h_time_norm / dt;        % 转换为连续冲激响应 h(t)，量纲为 s^-1
    
    results(s_idx).P_rx = P_rx_total;
    results(s_idx).Path_Loss_dB = 10 * log10(P_rx_total);
    
    if P_rx_total > 0
        % 时延拓展计算继续使用离散能量权重，dt在分子分母中自然抵消，逻辑严密
...
% 注意：此处调用 .h_time_cir 并将纵坐标单位标明为 dB s^{-1}
plot(param.T_bins*1e9, 10*log10(results(1).h_time_cir), 'r-', 'LineWidth', 1, 'DisplayName', 'With Turbulence');
hold on;
plot(param.T_bins*1e9, 10*log10(results(2).h_time_cir), 'b--', 'LineWidth', 1, 'DisplayName', 'Without Turbulence');
grid on;
...
% --- Plot 2: 归一化连续能量分布对比 (线性) ---
figure('Name', 'Linear Power Comparison', 'Color', 'w');
area(param.T_bins*1e9, results(2).h_time_cir, 'FaceColor', 'b', 'FaceAlpha', 0.3, 'EdgeColor', 'none', 'DisplayName', 'No Turb');
hold on;
plot(param.T_bins*1e9, results(1).h_time_cir, 'r-', 'LineWidth', 1.5, 'DisplayName', 'With Turb');
grid on;
legend;
xlabel('Time (ns)'); 
...
function [rho0_exact, Cn2_eq] = calc_turb_coherence_params(Phi_n_func, k_wave, L)
    % CALC_TURB_COHERENCE_PARAMS 从海洋 OTOPS 谱预计算光学相干参数
    % 
    % 返回值:
    %   rho0_exact: 基于严格球面波结构函数数值积分得到的相干长度 (m)
    %   Cn2_eq:     基于惯性子区渐近匹配反推的等效大气湍流强度 (m^(-2/3))

    %% 方法 A: 惯性子区渐近匹配法 (快速提取等效 Cn2)
    % 在惯性子区内，海洋 OTOPS 谱与 Kolmogorov 谱 Phi(k) = 0.033 * Cn2 * k^(-11/3) 拥有相似的幂律
    % 选择一个处于含能区与耗散区之间的空间频率 (如 kappa = 100 rad/m)
    kappa_eval = 100; 
    ...
    %% 方法 B: 严格波结构函数数值积分法 (物理最严谨)
    % 球面波相干长度 rho_0 的标准定义为波结构函数 D_w(rho_0) = 2 (即相干度降至 1/e)
    % D_w(rho) = 8 * pi^2 * k^2 * L * int_0^1 d(xi) int_0^inf dK [ K * Phi_n(K) * (1 - J0(K * rho * xi)) ]
    
    % 定义内部积分函数 calc_Dw(rho)
    % 为了加速数值积分，将 K 的上限限制在耗散区以外的高频截断 (如 1e4)
    calc_Dw = @(rho) 8 * pi^2 * k_wave^2 * L * integral2(...
        @(K, xi) K .* Phi_n_func(K) .* (1 - besselj(0, K .* rho .* xi)), ...
        1e-1, 1e4, ... % K 的积分区间 
        0, 1, ...      % xi 的积分区间 (0~1)
        'Method', 'iterated', 'RelTol', 1e-3);

    try
        % 使用 fzero 求解使得 D_w(rho) = 2 的精确 rho0
        opts = optimset('Display','off');
        rho0_exact = fzero(@(rho) calc_Dw(rho) - 2, rho_guess, opts);
    catch
        warning('二重数值积分寻根失败，退回使用等效 Cn2 解析近似值。');
        rho0_exact = rho_guess;
    end
end

function sigma2_I = calc_scintillation_OTOPS(Phi_n_func, k_wave, L)
    % 计算 OTOPS 谱下的球面波弱湍流闪烁指数 (Rytov Variance)
    % 
    % Phi_n_func: OTOPS谱函数句柄
    % k_wave: 波数 2*pi/lambda
    % L: 传输距离
    
    % 定义 Rytov 球面波积分核函数
    % 注意: K 为空间频率 \kappa，xi 为归一化距离 \xi
    integrand = @(K, xi) K .* Phi_n_func(K) .* ...
        (sin( K.^2 .* L .* xi .* (1 - xi) ./ (2 * k_wave) )).^2;
        
    % 数值积分 (对 K 的上限取一个足够大的截断值，如 1e4 覆盖耗散区)
    % 积分区间: K 从 1e-3 到 1e4, xi 从 0 到 1
    integral_val = integral2(integrand, 1e-3, 1e4, 0, 1, ...
        'Method', 'iterated', 'RelTol', 1e-4);
        
    % 乘以常数系数
    sigma2_I = 8 * pi^2 * k_wave^2 * L * integral_val;
end