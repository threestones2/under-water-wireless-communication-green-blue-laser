...
% --- 1. 加载所有产生的数据 ---
try
    d_WCI      = load('data_exp3_div_WCIMC.mat');
...
div_angles_deg = d_WCI.div_angles_deg;
...
for i = 1:num_div
    ...
    % W_geo = w0 + dist_axis .* tan(theta_half_rad);
    ...
    % 绘制性能曲线
    h1 = plot(dist_axis, -d_WCI.PL_Cell_WCI_Strong{i}, '-', 'Color', c_Strong, ...
                   'Marker', 'o', 'LineWidth', 1.5, 'MarkerSize', 7, 'MarkerFaceColor', c_Strong);
    h2 = plot(dist_axis, -d_WCI.PL_Cell_WCI_Weak{i}, '-', 'Color', c_Weak, ...
    ...
    y_text = -d_WCI.PL_Cell_WCI_Strong{i}(end);
...
for i = 1:num_div
    ...
    penalty_strong = (-d_WCI.PL_Cell_WCI_Strong{i}) - (-d_WCI_None.PL_Cell_WCI_None{i});
    penalty_weak   = (-d_WCI.PL_Cell_WCI_Weak{i}) - (-d_WCI_None.PL_Cell_WCI_None{i});
...
disp('数据可视化完毕。所有图像均按照要求 4:3 物理比例出图，已包含强/弱双重湍流对比。');