%% Exp3.3.1: WCI-MC Algorithm for Different Divergence Angles (Clear Ocean)
% 特性: 真实光子追踪，强/弱双重湍流耦合，内置所有底层标量化加速函数
clc; clear; close all;

% ================= 参数初始化 =================
N_packets = 1e5; % 蒙特卡洛光子数 (出图建议 1e6~1e7)
n_max = 200; 
lambda = 514e-9; k_wave = 2 * pi / lambda;

dist_axis = 5:5:60; 
num_dist = length(dist_axis);

w0 = 0.002;                         % 初始束腰半径
Rx_Aperture = 0.01;                 % 接收机孔径
Rx_FOV = 5 * pi / 180;            % 视场角
Rx_Area = pi * (Rx_Aperture / 2)^2;
cos_FOV_half = cos(Rx_FOV / 2);     
Rx_Aperture_half_sq = (Rx_Aperture / 2)^2;

% 水质固定为 Petzold Clear Ocean
...
div_angles_deg = [0, 0.1, 1];  % 自变量：三种束散角
...
PL_Cell_WCI_Strong = cell(1, num_div);
PL_Cell_WCI_Weak = cell(1, num_div);

fprintf('--- 运行 Exp3.3.1: WCI-MC (束散角遍历) ---\n');
...
for t_idx = 1:2
    ...
    for i = 1:num_div
        div_angle = div_angles_deg(i) * pi / 180;
        ...
        for d_idx = 1:num_dist
            ...
            for scr = 1:N_screens
                ...
                Grad_X_3D(:, :, scr) = gx; Grad_Y_3D(:, :, scr) = gy; Screen_Z_1D(scr) = scr * delta_z_screen; 
            end
...
save('data_exp3_div_WCIMC.mat', 'dist_axis', 'div_angles_deg', 'PL_Cell_WCI_Strong', 'PL_Cell_WCI_Weak', 'coef_c');
fprintf('已保存数据至 data_exp3_div_WCIMC.mat\n');
