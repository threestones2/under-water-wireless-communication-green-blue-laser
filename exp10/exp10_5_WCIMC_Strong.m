%% exp10_5: WCI-MC (Strong Turbulence) — 光子数扫描
%  强湍流 WCI-MC：弹道光子 + 散射虚拟光子 + 湍流相位屏 + Marcum Q 统计
%  与 exp10_1 (无湍流) 对比，衡量湍流效应（相位屏、光线步进等）的计算耗时增量
...
turb_params.name = 'Strong';

% 相位屏参数 (已验证：80m 清澈海水不发生相位卷绕)
D_screen = 0.2;          % 屏边长 0.2m (光束 + 漂移包络)
N_grid = 2^10;            
delta_z_target = 4;      % 目标屏间距 4m (≈ 80m / 20屏)
dx = D_screen / N_grid;  % 网格像素宽度

PL_Cell = cell(1, num_W);
Time_Cell = cell(1, num_W);

fprintf('--- exp10_5: WCI-MC (强湍流 + 相位屏) 光子数扫描 ---\n');
fprintf('湍流: %s | D_screen=%.1fm | N_grid=%d | dz≈%.0fm\n\n', ...
    turb_params.name, D_screen, N_grid, delta_z_target);
...
for w_idx = 1:num_W
    ...
    for d_idx = 1:num_D
        ...
        fprintf('  [屏] %s L=%2dm → N_screens=%d, dz=%.1fm, dx=%.2fmm\n', ...
            water_types{w_idx}, L, N_screens, delta_z_screen, dx*1e3);
            ...
            for p = 1:N_packets
                ...
                for ord = 1:n_max
                    ...
                    [p1, p2, p3, d1, d2, d3, ~, step_len] = ...
                        ...
                        Rx, Ry, Rz, Rx_Aperture_half_sq, cos_FOV_half, Nx, Ny, Nz, false, ...
                        Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, ...
                        k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);
                    ...
                    if base_w > 1e-16
                        [p1_v, p2_v, p3_v, d1_v, d2_v, d3_v, v_hit, v_len] = ...
                            ray_march_flat_turb(p1, p2, p3, dir2rx_1, dir2rx_2, dir2rx_3, ...
                            dist2rx+1e-1, Rx, Ry, Rz, 1e10, -1, Nx, Ny, Nz, true, ...
                            Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, ...
                            k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);
...
save('data_exp10_WCIMC_Strong.mat', 'dist_cell', 'PL_Cell', 'Time_Cell', 'N_packets_arr', 'water_types');
fprintf('已保存至 data_exp10_WCIMC_Strong.mat\n');
