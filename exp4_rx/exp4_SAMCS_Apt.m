%% Exp4_2: Standard MCS Algorithm for Different Receiver Apertures
clc; clear; close all;
...
for i = 1:num_apt
    ...
    for d_idx = 1:num_dist
        ...
        P_mcs_accum = 0; tic; 
        rng(123456 + L*100, 'twister'); % 确保每个距离种子独立
        
        for p = 1:N_packets
            r0 = w0 * sqrt(-0.5*log(rand())); phi0 = 2*pi*rand(); 
            p1_mcs = Tx + r0*cos(phi0); p2_mcs = Ty; p3_mcs = Tz + r0*sin(phi0);
            
            U_init = theta_half_div * sqrt(-0.5 * log(rand())); 
            dir = rotate_direction_fast([Lx,Ly,Lz], cos(U_init), sin(U_init), 2*pi*rand());
            d1_mcs = dir(1); d2_mcs = dir(2); d3_mcs = dir(3);
            w_mcs = 1.0; P_packet_mcs = 0;
            
            % 直射光 (Zero-order hit)
            cos_th = d1_mcs*Lx + d2_mcs*Ly + d3_mcs*Lz;
            if cos_th > 0
                d_plane = ((Rx - p1_mcs)*Lx + (Ry - p2_mcs)*Ly + (Rz - p3_mcs)*Lz) / cos_th; 
                pos_end_1 = p1_mcs + d1_mcs * d_plane; pos_end_2 = p2_mcs + d2_mcs * d_plane; pos_end_3 = p3_mcs + d3_mcs * d_plane; 
                if abs(d1_mcs*Nx + d2_mcs*Ny + d3_mcs*Nz) >= cos_FOV_half && (pos_end_1 - Rx)^2 + (pos_end_2 - Ry)^2 + (pos_end_3 - Rz)^2 <= Rx_Aperture_half_sq
                    P_packet_mcs = exp(-param.coef_c * d_plane);
                end
            end
            ...
            for ord = 1:n_max
                ...
                p1_mcs = p1_mcs + d1_mcs * d_step; p2_mcs = p2_mcs + d2_mcs * d_step; p3_mcs = p3_mcs + d3_mcs * d_step; 
                w_mcs = w_mcs * exp(-param.coef_a * d_step);
                
                if (p1_mcs - Tx)*Lx + (p2_mcs - Ty)*Ly + (p3_mcs - Tz)*Lz >= L || w_mcs < 1e-15
                    break; 
                end 
                
                vec2rx_1 = Rx - p1_mcs; vec2rx_2 = Ry - p2_mcs; vec2rx_3 = Rz - p3_mcs; 
                ...
                if cos_inc >= cos_FOV_half
                    ...
                    cos_scatter = d1_mcs*dir2rx_1 + d2_mcs*dir2rx_2 + d3_mcs*dir2rx_3;
                    P_packet_mcs = P_packet_mcs + w_mcs * min(1, pdf_Empirical(cos_scatter, param) * omega) * exp(-param.coef_c * d2rx); 
                end
                
                u_rand = rand(); idx_lut = floor(u_rand * (LUT_SIZE - 1)) + 1; 
                ct_s = cos_th_LUT(idx_lut); st_s = sin_th_LUT(idx_lut);
                dir_new = rotate_direction_fast([d1_mcs, d2_mcs, d3_mcs], ct_s, st_s, 2*pi*rand()); 
                d1_mcs = dir_new(1); d2_mcs = dir_new(2); d3_mcs = dir_new(3);
            end
            P_mcs_accum = P_mcs_accum + P_packet_mcs;
        end
        PL_MCS_temp(d_idx) = 10 * log10(max(P_mcs_accum / N_packets, 1e-300));
        fprintf('    L=%2dm | Time: %5.2fs | MCS: %6.2f dB\n', L, toc, -PL_MCS_temp(d_idx));
    end
    PL_Cell_MCS{i} = PL_MCS_temp;
end
