%% Exp4_3: Data Visualization (Path Loss & Increments vs Aperture - Strong/MCS Only)
clc; clear; close all;

try
    d_WCI = load('data_exp4_apt_WCIMC_Strong.mat');
    d_MCS = load('data_exp4_apt_MCS.mat');
catch
    error('数据加载失败，请务必先运行 WCI-MC (Strong) 和 MCS 仿真脚本生成 .mat 数据文件。');
end
...
h_alg2 = plot(nan, nan, '-', 'Color', c_MCS, 'Marker', '^', 'LineWidth', 1.5, 'MarkerFaceColor', c_MCS);
h_alg3 = plot(nan, nan, '-', 'Color', c_Beer, 'Marker', '*', 'LineWidth', 1.5, 'MarkerFaceColor', c_Beer);
...
legend([h_alg1, h_alg2, h_alg3, h_apt1, h_apt2, h_apt3], ...
    {'WCI-MC (Strong)', 'SA-MCS (w/o Turb)', 'Beer Law', ...
...
% =========================================================================
% 图 2：核心机制对比：强湍流相对于 SA-MCS 的衰减增量演化图
% =========================================================================
...
for i = 1:num_apt
    ...
    h_leg_all = [h_leg_all, h_strong(i)];
end
...
title('Aperture Averaging Effect on Strong Turbulence Penalty', 'FontSize', 15, 'FontName', 'Times New Roman');
xlim([min(dist_axis), max(dist_axis)]);

legend(h_leg_all, leg_labels_inc, 'Location', 'northwest', 'FontSize', 11, 'NumColumns', 1, 'FontName', 'Times New Roman');