%% Exp4_1: WCI-MC Algorithm for Different Receiver Apertures (Weak Turbulence)
clc; clear; close all;
...
turb_params.epsilon = 1e-9; turb_params.chi_T = 1e-7; turb_params.name = 'Weak';

N_screens = 20; D_screen = 1; N_grid = 2^8; dx = D_screen / N_grid; x_axis = (-N_grid/2 : N_grid/2-1) * dx;

PL_Cell_WCI_Weak = cell(1, num_apt);
            ...
            for ord = 1:n_max
                ...
                if base_w > 1e-16
                    ...
                    if v_hit
                        ...
                        % if W_ST > 1e-6, 
                        %     point_loss = 1 - marcumq(2*r_wp/W_ST, 2*r_eff_v/W_ST); 
                        % else, 
...
save('data_exp4_apt_WCIMC_Weak.mat', 'dist_axis', 'apertures_m', 'PL_Cell_WCI_Weak');
