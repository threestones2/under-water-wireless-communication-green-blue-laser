%% Exp1: MCI-PIS (Uniform Sampling, Zero-Allocation Scalarized)
clc; clear; close all;
...
param.coef_a = 0.179; param.coef_b = 0.219; param.coef_c = 0.398; param.albedo = param.coef_b / param.coef_c;
...
fprintf('--- 运行 MCI-PIS 收敛性测试 (零分配标量加速) ---\n');
for idx = 1:num_N
    ...
    for p = 1:N_packets
        ...
        for ord = 1:10
            ...
            ct_i = cos(th_i); st_i = sin(th_i);
            
            weight = weight * min(1.0, pdf_Empirical(ct_i, param) * 2 * pi^2 * st_i);
            
            dir = rotate_direction_fast([d1, d2, d3], ct_i, st_i, phi_i);
            d1 = dir(1); d2 = dir(2); d3 = dir(3);
        end
...
save('data_exp1_MCIPIS.mat', 'N_arr', 'PL_arr', 'Time_arr');
