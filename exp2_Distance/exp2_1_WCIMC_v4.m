%% Exp2: WCI-MC (Wavefront-Coupled + Adaptive HG-PIS, FOV Spatial Truncation)
% 物理机制: 包含系综平均 (Ensemble Averaging) 以提取湍流宏观展宽损耗
% 优化声明: 引入 FOV 空间截断剔除策略，切断无效散射路径算力消耗
...
N_packets = 1e4;                     % 单个脉冲的光子数
n_max = 200;                         % 最大散射阶数
N_realizations = 2;                % 独立湍流系综实现次数 (消除闪烁误差)
...
fprintf('--- 运行 WCI-MC 信道路径损耗评估 (100次系综平均) ---\n');
    ...
    for w_idx = 1:num_W
        ...
        for d_idx = 1:num_D
            ...
            for real_idx = 1:N_realizations
                % 保证单次循环及复现时的随机独立性
                rng(123456 + t_idx*1e5 + w_idx*1e4 + L*100 + real_idx, 'twister');
                ...
                for p = 1:N_packets
                    ...
                    [p1_end, p2_end, p3_end, d1_end, d2_end, d3_end, plane_hit, path_len_ballistic] = ...
                        ray_march_flat_scalar(p1, p2, p3, d1, d2, d3, 1e9, Rx, Ry, Rz, 1e9, -1, Nx, Ny, Nz, true, Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);
                    ...
                    for ord = 1:n_max
                        ...
                        if base_w > 1e-15
                            [p1_v, p2_v, p3_v, d1_v, d2_v, d3_v, v_hit, v_len] = ...
                                ray_march_flat_scalar(p1, p2, p3, dir2rx_1, dir2rx_2, dir2rx_3, dist2rx+1e-1, Rx, Ry, Rz, 1e10, -1, Nx, Ny, Nz, true, Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);
                            if v_hit
                ...
                % 记录该次独立湍流实现下提取的归一化线性功率
                P_rx_realizations(real_idx) = P_rx_accum / N_packets;
            end
            
            % --- 对线性功率求取系综平均，转化为对数衰减 ---
            avg_P_rx = mean(P_rx_realizations);
            PL_arr_temp(d_idx) = 10 * log10(max(avg_P_rx, 1e-300));
            
            fprintf('    距离 L = %2dm | 计算耗时: %6.1fs | 系综平均 Path Loss: %6.2f dB\n', L, toc, -PL_arr_temp(d_idx));
        end
        
        if t_idx == 1
            PL_Cell_WCI_Strong{w_idx} = PL_arr_temp;
        else
            PL_Cell_WCI_Weak{w_idx} = PL_arr_temp;
        end
    end
end
save('data_exp2_WCIMC_Ensemble.mat', 'dist_cell', 'PL_Cell_WCI_Strong', 'PL_Cell_WCI_Weak', 'water_types');
fprintf('数据保存完成: data_exp2_WCIMC_Ensemble.mat\n');
