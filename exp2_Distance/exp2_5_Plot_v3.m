%% Exp2: Plotting Macroscopic Channel Characteristics (Updated with WCI-MC None & SA-MCS)
clc; clear; close all;
        
try
    d_WCI  = load('data_exp2_WCIMC.mat');
    d_None = load('data_exp2_WCIMC_None.mat'); % 替换为无湍流基准数据
...
xlim([5, 63]); 
% 更新图例名称 MCS -> SA-MCS
leg_labels = {'WCI-MC (w/ Strong)', 'WCI-MC (w/ Weak)', 'WCI-MC (w/o)', 'SA-MCS', 'Beer'};
...
plot(ax_inset, dist_zoom, -d_WCI.PL_Cell_WCI_Weak{w_zoom}, '-', 'Color', c_Weak, ...
    'Marker', 'd', 'LineWidth', 1.5, 'MarkerSize', 7, 'MarkerFaceColor', 'w');
% 替换为无湍流数据的调用
plot(ax_inset, dist_zoom, -d_None.PL_Cell_WCI_None{w_zoom}, '--', 'Color', c_NoTurb, ...
    'Marker', 's', 'LineWidth', 1.5, 'MarkerSize', 7, 'MarkerFaceColor', c_NoTurb);
...
% diff_Strong_NoTurb = PL_Strong - PL_MCS;
% diff_Weak_NoTurb   = PL_Weak - PL_MCS;  
% diff_Strong_Weak   = PL_Strong - PL_Weak;
