%% Exp2_PDF: Turbulence Intensity PDF Simulation (200 Realizations)
% 物理机制: 固定传输距离，通过 200 次独立相位屏生成，统计短期接收光强波动
% 语言风格: 科学、客观、严谨
...
% ================= 参数初始化 =================
L = 10;                              % 固定传输距离 (m)。设为 10m 以保证港口水质下仍有足够统计量
N_packets = 2000;                    % 单次实现的光子数 (代表极短时间窗口内的一个光脉冲)
N_realizations = 200;                % 独立湍流相位屏的实现次数
...
w0 = 0.002;                          % 初始束腰半径
theta_half_div = 0.1 * pi / 180 / 2; % 半发散角
Rx_Aperture = 0.05;                  % 接收机孔径 50 mm
Rx_FOV = 5 * pi / 180;             
...
% 湍流环境设定
turb_params = struct();
turb_params(1).T_avg = 20; turb_params(1).S_avg = 35; turb_params(1).H_ratio = -1; 
turb_params(1).epsilon = 1e-10; turb_params(1).chi_T = 1e-5; turb_params(1).name = 'Strong';
turb_params(2).T_avg = 20; turb_params(2).S_avg = 35; turb_params(2).H_ratio = -20; 
turb_params(2).epsilon = 1e-9; turb_params(2).chi_T = 1e-7; turb_params(2).name = 'Weak';
num_T = length(turb_params);

N_screens = 10; % 为加快仿真，屏幕数适度下调
D_screen = 1; N_grid = 2^8; dx = D_screen / N_grid; x_axis = (-N_grid/2 : N_grid/2-1) * dx;

% 数据存储阵列: (湍流类型, 水质类型, 实现次数)
Intensity_Samples = zeros(num_T, num_W, N_realizations);

fprintf('--- 运行光强概率密度(PDF)统计仿真 ---\n');
fprintf('距离: %d m | 光子数/脉冲: %d | 采样次数: %d\n', L, N_packets, N_realizations);
...
for w = 1:num_W
    ...
    for t_idx = 1:num_T
        ...
        for real_idx = 1:N_realizations
            % 强制变更随机种子，确保每次内循环生成不同的湍流相位屏
            rng(real_idx * 1000 + w * 10 + t_idx, 'twister'); 
            ...
            for p = 1:N_packets
                ...
                for ord = 1:n_max
                    ...
                    [p1, p2, p3, d1, d2, d3, ~, step_len] = ...
                        ray_march_flat_scalar_v3(p1, p2, p3, d1, d2, d3, d_step, Rx, Ry, Rz, Rx_Aperture_half_sq, cos_FOV_half, Nx, Ny, Nz, false, Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen, N_screens);
                    ...
                    if cos_inc >= cos_FOV_half
                        ...
                        if base_w > 1e-15
                            [p1_v, p2_v, p3_v, d1_v, d2_v, d3_v, v_hit, v_len] = ...
                                ray_march_flat_scalar_v3(p1, p2, p3, dir2rx_1, dir2rx_2, dir2rx_3, dist2rx+1e-1, Rx, Ry, Rz, 1e10, -1, Nx, Ny, Nz, true, Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen, N_screens);
                            if v_hit
                                cos_inc_v = abs(d1_v*Nx + d2_v*Ny + d3_v*Nz); r_eff_v = (Rx_Aperture/2) * sqrt(cos_inc_v);
                                vec_x = p1_v - Rx; vec_y = p2_v - Ry; vec_z = p3_v - Rz;
                                cx = vec_y*d3_v - vec_z*d2_v; cy = vec_z*d1_v - vec_x*d3_v; cz = vec_x*d2_v - vec_y*d1_v;
            ...
            Intensity_Samples(t_idx, w, real_idx) = P_rx_accum / N_packets;

            % 进度提示
            if mod(real_idx, 50) == 0
                fprintf('    完成 %d/%d 次采样...\n', real_idx, N_realizations);
            end
        end
        fprintf('  -> 本项耗时: %.2f s\n', toc);
    end
...
% 保存计算数据
save('data_exp2_PDF.mat', 'Intensity_Samples', 'turb_params', 'water_types', 'L');

%% ================= PDF 统计分析与对数正态分布拟合 (散点版) =================

colors = {'#D95319', '#0072BD'}; % 强湍流为橘红，弱湍流为蓝色
markers = {'o', 'd'};            % 强湍流用圆圈，弱湍流用菱形

for w = 1:num_W
    % 为每一种水质单独创建一个图形窗口
    figure('Name', ['Intensity PDF - ', water_types{w}], 'Color', 'w', ...
           'Position', [100 + w*50, 100 + w*50, 520, 420]);
    hold on;
    
    for t_idx = 1:num_T
        % 获取 200 个样本并计算统计量
        samples = squeeze(Intensity_Samples(t_idx, w, :));
        mean_I = mean(samples);
        ...
        % 绘制仿真数据点 (只画点，不连线)
        plot(bin_centers, pdf_counts, markers{t_idx}, 'Color', colors{t_idx}, ...
             'MarkerSize', 7, 'LineWidth', 1.5, 'MarkerFaceColor', 'none', ...
             'DisplayName', sprintf('Sim. %s', turb_params(t_idx).name));
        
        % ----------------------------------------------------
        % 理论对数正态分布 (Lognormal PDF) 连续曲线
        % ----------------------------------------------------
        ...
        if SI > 0
            ...
            % 绘制理论拟合曲线 (实线)
            plot(I_axis, PDF_theo, '-', 'Color', colors{t_idx}, 'LineWidth', 1.5, ...
                 'DisplayName', sprintf('Theo. %s (SI=%.3f)', turb_params(t_idx).name, SI));
        end
    end
    ...
    title(['Received Intensity PDF: ', water_types{w}], 'FontSize', 14, 'FontName', 'Times New Roman');
