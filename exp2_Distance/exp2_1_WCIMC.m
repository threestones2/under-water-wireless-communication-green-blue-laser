%% Exp2: WCI-MC (Wavefront-Coupled + Adaptive HG-PIS, With Rigorous Phase Screens)
clc; clear; close all;

% ================= 物理参数初始化 =================
dist_cell = {5:10:55, 5:10:55, 5:5:20}; 
...
w0 = 0.002; 
div_angle = 20 * pi / 180; theta_half_div = div_angle / 2; % 宽波束发散 20度
Rx_Aperture = 0.2; Rx_FOV = 90 * pi / 180; Rx_Area = pi * (Rx_Aperture / 2)^2; % 宽视场大孔径
...
fprintf('--- 运行 Exp2: WCI-MC (含严格相位屏湍流, 宽波束配置) ---\n');
...
for w_idx = 1:num_W
    ...
    % % 计算水体本征展宽角，已无用，删除
    % th_test_w = linspace(1e-6, pi/4, 2000); p_test = zeros(size(th_test_w));
    % for idx = 1:length(th_test_w), p_test(idx) = pdf_Empirical(cos(th_test_w(idx)), param); end
    ...
    for d_idx = 1:num_D
        ...
        Screen_Chain = repmat(struct('Center',[], 'Normal',[0,1,0], 'u_vec',u_vec_Tx, 'v_vec',v_vec_Tx, 'grad_x',[], 'grad_y',[]), 1, N_screens);
        for i = 1:N_screens
        ...
        for p = 1:N_packets
            ...
            % --- 0阶直射路径 ---
            [pos_end, dir_end, plane_hit, path_len_ballistic] = ray_march_generic(pos, dir, 1e9, Rx_Pos, 1e5, pi, Rx_Normal, true, Screen_Chain, k_wave, x_axis, dx, N_grid, Tx_Pos, Link_Dir, delta_z_screen);
            if plane_hit
            ...
            for ord = 1:n_max
                ...
                if acos(dot(-dir2rx, Rx_Normal)) <= Rx_FOV/2
                    omega = Rx_Area / (dist2rx^2) * abs(dot(dir2rx, Rx_Normal));
                    base_w = weight * min(1, pdf_Empirical(dot(dir, dir2rx), param) * omega) * exp(-param.coef_c * dist2rx);
                    
                    if base_w > 1e-15
                        [pos_v, ~, v_hit, v_len] = ray_march_generic(pos, dir2rx, dist2rx+1e-1, Rx_Pos, 1e5, pi, Rx_Normal, true, Screen_Chain, k_wave, x_axis, dx, N_grid, Tx_Pos, Link_Dir, delta_z_screen);
                        if v_hit
                            cos_inc = abs(dot(dir2rx, Rx_Normal)); r_eff_v = (Rx_Aperture/2)*sqrt(cos_inc);
                            r_wp = norm(cross(pos_v - Rx_Pos, dir2rx));
...
function nd = rotate_direction(d, t, p)
    ...
    if denom < 1e-10
        ...
        nd = [sin(t)/denom*(d(1)*d(3)*cos(p)-d(2)*sin(p))+d(1)*cos(t), sin(t)/denom*(d(2)*d(3)*cos(p)+d(1)*sin(p))+d(2)*cos(t), -sin(t)*cos(p)*denom+d(3)*cos(t)]; 
    end
    nd = nd / norm(nd);
end
%
function [pos, dir, hit_flag, total_len] = ray_march_generic(pos, dir, dist_limit, Rx_Pos, Rx_Aperture, Rx_FOV, Rx_Normal, enable_hit_check, Screen_Chain, k_wave, x_axis, dx, N_grid, Tx_Pos, Link_Dir, delta_z_screen)
    ...
    while remaining_dist > 1e-6
        ...
        if dot(dir, Link_Dir) > 1e-6
            target_scr = current_layer + 1; 
        elseif dot(dir, Link_Dir) < -1e-6
            target_scr = current_layer; 
        else
            target_scr = -1; 
        end

        if target_scr >= 1 && target_scr <= N_screens
            scr = Screen_Chain(target_scr);
        ...
        elseif strcmp(event_type, 'screen')
            ...
            idx_x = mod(round((dot(vec_p, scr.u_vec) - x_axis(1))/dx), N_grid) + 1; 
            idx_y = mod(round((dot(vec_p, scr.v_vec) - x_axis(1))/dx), N_grid) + 1;
            dir = dir + (scr.grad_x(idx_y, idx_x)*scr.u_vec + scr.grad_y(idx_y, idx_x)*scr.v_vec)/k_wave; 
            dir = dir / norm(dir);
        end
    end
end
