%% Exp2: WCI-MC (Wavefront-Coupled + Adaptive HG-PIS) - PDF Distribution
% 功能: 模拟 20m 和 50m 的归一化光强 PDF，将两种距离绘制在同一张图中进行对比

clc; clear; close all;
cd(fileparts(mfilename('fullpath')));

% ================= 物理参数初始化 =================
% 将测试距离扩展为包含 20m 和 50m
dist_cell = {[20, 50]}; 
water_types = {'Clear Ocean'};
coef_a_arr = [0.114]; 
coef_b_arr = [0.0374]; 
coef_c_arr = [0.1514];
...
N_packets = 2e4;                     
n_max = 200;                         
N_realizations = 200; % 建议保持较高次数以获得平滑的 PDF 直方图               
...
% ================= 湍流环境配置 =================
tp.T_avg = 25; tp.S_avg = 35; tp.H_ratio = -0.3; 
tp.epsilon = 1e-4; tp.chi_T = 1e-7; tp.name = 'Weak';

N_screens = 20; 
D_screen = 1; N_grid = 2^10; dx = D_screen / N_grid; x_axis = (-N_grid/2 : N_grid/2-1) * dx;

fprintf('--- 运行 WCI-MC 闪烁指数(SI)与光强 PDF 联合评估 ---\n');
...
for w_idx = 1:num_W
    ...
    % 用于暂存不同距离下归一化光强的数据结构
    I_norm_all = cell(1, num_D);
    SI_all = zeros(1, num_D);
    ...
    for d_idx = 1:num_D
        ...
        P_rx_realizations = zeros(1, N_realizations); 
        tic;
        
        for real_idx = 1:N_realizations
            rng(123456 + w_idx*1e4 + L*100 + real_idx, 'twister');
            ...
            for i = 1:N_screens
                ...
                Grad_X_3D(:, :, i) = gx; Grad_Y_3D(:, :, i) = gy; Screen_Z_1D(i) = i * delta_z_screen; 
            end
            ...
            for p = 1:N_packets
                ...
                [p1_end, p2_end, p3_end, d1_end, d2_end, d3_end, plane_hit, path_len_ballistic] = ...
                    ray_march_flat_scalar_v3(p1, p2, p3, d1, d2, d3, 1e9, Rx, Ry, Rz, Rx_Aperture_half_sq, cos_FOV_half, Nx, Ny, Nz, true, Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen, N_screens);
                ...
                for ord = 1:n_max
                    ...
                    base_w = weight * min(1, pdf_Empirical(d1*dir2rx_1 + d2*dir2rx_2 + d3*dir2rx_3, param) * (Rx_Area / d2rx_sq * cos_inc)) * exp(-param.coef_c * dist2rx);
                    if base_w > 1e-15
                    ...
                    weight = weight * max(0.5,min(2, pdf_Empirical(cos_th_i, param) / ((1 - g_prop^2) / (4 * pi * (1 + g_prop^2 - 2 * g_prop * cos_th_i)^1.5))));
            ...
            P_rx_realizations(real_idx) = P_rx_accum / N_packets;
        end
        
        % 计算该距离下的归一化光强与闪烁指数，并存入数组
        I_norm = P_rx_realizations / mean(P_rx_realizations);
        I_norm_all{d_idx} = I_norm;
        SI_all(d_idx) = var(I_norm);
        
        fprintf('    距离 L = %2dm | 耗时: %6.1fs | SI (闪烁指数): %6.4f\n', L, toc, SI_all(d_idx));
    end
    
    % ================= 联合绘图模块 (PDF) =================
    figure('Name', 'PDF of Normalized Irradiance', 'Position', [150, 150, 750, 500]);
    hold on; box on; grid on;
    
    colors = {'#D95319', '#0072BD'}; % 分别对应 20m 和 50m 的颜色 (红和蓝)
    
    for d_idx = 1:num_D
        I_norm = I_norm_all{d_idx};
        L = dist_arr(d_idx);
        c = colors{d_idx};
        
        % 计算 PDF 值并以散点（'o'）绘制，避免直方图色块遮挡
        [counts, edges] = histcounts(I_norm, 15, 'Normalization', 'pdf');
        centers = (edges(1:end-1) + edges(2:end)) / 2;
        plot(centers, counts, 'o', 'MarkerEdgeColor', c, 'MarkerFaceColor', 'none', ...
            'LineWidth', 1.5, 'DisplayName', sprintf('Simulation data (L=%dm)', L));
              
        % 进行对数正态 (Lognormal) 拟合，并绘制理论曲线
        pd = fitdist(I_norm', 'Lognormal');
        x_vals = linspace(min(I_norm)*0.5, max(I_norm)*1.5, 300);
        y_pdf = pdf(pd, x_vals);
        
        plot(x_vals, y_pdf, '-', 'Color', c, 'LineWidth', 2.0, ...
             'DisplayName', sprintf('Lognormal Fit (L=%dm)', L));
    end
    
    xlabel('Normalized Irradiance, $I / \langle I \rangle$', 'Interpreter', 'latex', 'FontSize', 13);
    ylabel('Probability Density $p(I)$', 'Interpreter', 'latex', 'FontSize', 13);
    title('PDF of Normalized Irradiance at 20m and 50m', 'FontSize', 14);
