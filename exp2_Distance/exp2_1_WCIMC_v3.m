...
% 优化声明: 
% 1. 实施无分配标量展开，极速 O(1) 提取，消灭所有数组分配开销
% 2. 物理修正: 引入 FOV 空间截断剔除策略，彻底切断无效散射路径的算力浪费
% 3. 新增: 强/弱双重光学湍流模式连续仿真与数据独立存储
clc; clear; close all;
...
% [2] 弱湍流参数 (Weak)
% turb_params(2).T_avg = 20; turb_params(2).S_avg = 35; turb_params(2).H_ratio = -1; 
% turb_params(2).epsilon = 1e-9; turb_params(2).chi_T = 1e-7; turb_params(2).name = 'Weak';

turb_params(2).T_avg = 20; turb_params(2).S_avg = 35; turb_params(2).H_ratio = -0.3; 
turb_params(2).epsilon = 1e-4; turb_params(2).chi_T = 1e-7; turb_params(2).name = 'Weak';

% 需要根据需要动态调节相位屏尺寸和
N_screens = 20; D_screen = 1; N_grid = 2^10; dx = D_screen / N_grid; x_axis = (-N_grid/2 : N_grid/2-1) * dx;

PL_Cell_WCI_Strong = cell(1, num_W);
PL_Cell_WCI_Weak = cell(1, num_W);

fprintf('--- 运行 Exp2: WCI-MC (FOV 空间截断优化版) ---\n');
...
for t_idx = 1:2
    ...
    for w_idx = 1:num_W
        ...
        for d_idx = 1:num_D
            L = dist_arr(d_idx);
            rng(123456 + L, 'twister');%保证不同算法比较不会因为生成相位屏差异导致的噪声干扰
            ...
            for p = 1:N_packets
                ...
                if plane_hit
                    ...
                    if cos_rx_tilt >= cos_FOV_half
                        ...
                        if W_spot > 1e-6, 
                            ...
                            recv_frac = double(r_wander_perp <= r_eff); end
                ...
                for ord = 1:n_max
                    ...
                    % 只有被确认为在 FOV 内的光子，才执行 PIS 强制接收与波前补偿
...
save('data_exp2_WCIMC.mat', 'dist_cell', 'PL_Cell_WCI_Strong', 'PL_Cell_WCI_Weak', 'water_types');
fprintf('已成功保存所有强/弱湍流数据至 data_exp2_WCIMC.mat\n');
