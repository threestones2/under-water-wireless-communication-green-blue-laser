%% MCS_Physical: Pure Physical Reception Monte Carlo Simulation
%  直射与散射使用完全一致的接收判断：
%    1. 光子是否穿越接收平面
%    2. 入射角是否在 FOV 内
%    3. 落点是否在接收孔径内
%  湍流通过相位屏 ray_march 自然作用，无 Marcum Q 统计模型。
...
N_packets = 1e6;         % 纯物理接收需要更多光子（无重要性采样）
n_max = 200;
...
N_screens = 20; D_screen = 1; N_grid = 2^10; dx = D_screen / N_grid;
x_axis = (-N_grid/2 : N_grid/2-1) * dx;

PL_Cell_MCS = cell(1, num_W);

fprintf('--- MCS_Physical (纯物理接收, 强湍流) ---\n');
fprintf('光子数: %d | 相位屏: %d x %d\n\n', N_packets, N_screens, N_grid);
...
for w_idx = 1:num_W
    ...
    for d_idx = 1:num_D
        ...
        for p = 1:N_packets
            ...
            % 保存初始位置和方向 (弹道检查后会修改，散射循环需恢复)
            p1_0 = p1; p2_0 = p2; p3_0 = p3;
            d1_0 = d1; d2_0 = d2; d3_0 = d3;

            % --- 弹道检查 (散射阶数 0)：使用无限距离确保走到接收平面 ---
            ...
            for ord = 1:n_max
                ...
                % 行进并检测接收平面 (enable_hit_check=true, 使用真实孔径和FOV)
                ...
                % 吸收衰减 (a 已通过 weight 追踪)
                weight = weight * exp(-param.coef_a * step_len);

                % 命中接收孔径 + FOV 内 → 计入贡献
                % 只需补上散射损耗 exp(-b * step_len)，避免 a 重复计算
                if hit_flag
                ...
                % --- O(1) LUT 散射角采样 (直接抽样经验相函数，无需权重修正) ---
...
save('data_MCS_Physical.mat', 'dist_cell', 'PL_Cell_MCS', 'water_types');
fprintf('已保存纯物理接收数据至 data_MCS_Physical.mat\n');
...
function [Phi_n_func, k_wave, eta_physical] = get_OTOPS_spectrum_handle(lambda, T, S, epsilon, chi_T, H_ratio)
    ...
    rho = (9.9992293295e2 + 2.0341179217e-2 * T - 6.1624591598e-3 * T^2 + ...
           ...
           s_f * (8.0200240891e2 - 2.0005183488 * T + 1.6771024982e-2 * T^2 - ...
           3.0600536746e-5 * T^3 - 1.6132224742e-5 * T * S);
    mu = ((0.15700386464 * (T + 64.992620050)^2 - 91.296496657)^(-1) + 4.2844324477e-5) * ...
         (1 + (1.5409136040 + 1.9981117208e-2 * T - 9.5203865864e-5 * T^2) * s_f + ...
         (7.9739318223 - 7.561456881e-2 * T + 4.7237011074e-4 * T^2) * s_f^2);
    sigma_T = 10^(log10(240 + 0.0002 * (S / 1.00472)) - 3 + ...
        0.434 * (2.3 - (343.5 + 0.037 * (S / 1.00472)) / (1.00024 * T + 273.15)) * ...
        (1 - (1.00024 * T + 273.15) / (647.3 + 0.03 * (S / 1.00472)))^(1/3));
    Pr = mu * cp / sigma_T;
    Sc = mu^2 / (5.954e-15 * T_k * rho);
    c_T = 0.072^(4/3) / Pr; c_S = 0.072^(4/3) / Sc;
    ...
    Phi_Hill = @(K, chi_M, c_M) (0.72 / (4 * pi)) * chi_M * epsilon^(-1/3) .* ...
        (K.^2 + 1e-25).^(-11/6) .* ...
        exp(-176.90 * (K * eta_physical).^2 .* c_M^(0.96)) .* ...
        (1 + 21.61 * (K * eta_physical).^(0.61) .* c_M^(0.02) - ...
         18.18 * (K * eta_physical).^(0.55) .* c_M^(0.04));
...
function phase_screen = gen_screen_from_spectrum(Phi_n_func, D, N, k_wave, delta_z)
    ...
    C_nm = (randn(N)+1i*randn(N)) .* ...
           sqrt(2*pi*k_wave^2*delta_z*Phi_n_func(K_grid)) * dk;
        ...
        for m = -1:1
            for n = -1:1
                ...
                phase_low = phase_low + real((randn(1)+1i*randn(1)) * ...
                    sqrt(2*pi^2*k_wave^2*delta_z*Phi_n_func(k_p)) * dk_p * ...
                    exp(1i*(m*dk_p*xx + n*dk_p*yy)));
            end
        end
    end
...
function [rho0_exact, Cn2_eq] = calc_turb_coherence_params(Phi_n_func, k_wave, L, eta_physical)
    ...
    try
        ...
        rho0_exact = fzero(@(rho) 8*pi^2*k_wave^2*L*integral2(...
            @(K, xi) K.*Phi_n_func(K).*(1-besselj(0, K.*rho.*xi)), ...
            1e-2, 1e5, 0, 1, 'Method', 'iterated', 'RelTol', 1e-3) - 2, ...
