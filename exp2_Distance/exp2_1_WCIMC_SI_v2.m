%% Exp2_PDF: Turbulence Intensity PDF Simulation (Single Condition)
% 物理机制: 固定传输距离，通过独立相位屏生成，统计短期接收光强波动
% 修改说明: 增加了水质与湍流强度的独立选择开关，单次运行仅计算和绘制一种特定组合。
% 扩展说明: 增加距离数组 [20, 50]，并在同一张图内绘制两组距离的 PDF 分布。
clc; clear; close all;
...
% ================= 参数初始化 =================
dist_arr = [20, 50];                 % 传输距离数组 (m)
num_D = length(dist_arr);
N_packets = 2000;                    % 单次实现的光子数 
...
% 修改全局湍流环境设定
turb_params_all(1).T_avg = 20; 
turb_params_all(1).S_avg = 35; 
turb_params_all(1).H_ratio = -0.3;     % 对应论文中的 w = -0.3
turb_params_all(1).epsilon = 1e-4;     % 湍动能耗散率
turb_params_all(1).chi_T = 1e-7;       % 温度均方耗散率
turb_params_all(1).name = 'Paper_Fig2a';
...
% 数据存储阵列: (特定湍流, 特定水质, 距离索引, 实现次数)
Intensity_Samples = zeros(num_T, num_W, num_D, N_realizations);

fprintf('--- 运行光强概率密度(PDF)统计仿真 (单一条件, 20m & 50m) ---\n');
fprintf('光子数/脉冲: %d | 采样次数: %d\n', N_packets, N_realizations);
fprintf('选择水质: %s | 湍流强度: %s\n', water_types{1}, turb_params(1).name);
    ...
    for t_idx = 1:num_T
        ...
        for d_idx = 1:num_D
            ...
            for real_idx = 1:N_realizations
                % 随机种子加入距离因子保证不同距离下序列的独立性
                rng(real_idx * 1000 + WATER_FLAG * 10 + TURB_FLAG + d_idx * 100, 'twister'); 
                ...
                Intensity_Samples(1, 1, d_idx, real_idx) = P_rx_accum / N_packets;
                if mod(real_idx, 50) == 0
                    fprintf('    完成 %d/%d 次采样...\n', real_idx, N_realizations);
                end
            end
            fprintf('  -> 距离 %d m 耗时: %.2f s\n', L, toc);
        end
    end
end

% 保存计算数据 (更新了保存的变量，包含 dist_arr)
save('data_exp2_PDF_single.mat', 'Intensity_Samples', 'turb_params', 'water_types', 'dist_arr');

%% ================= PDF 统计分析与对数正态分布拟合 (合并版) =================
figure('Name', ['Intensity PDF - ', water_types{1}], 'Color', 'w', ...
       'Position', [150, 150, 600, 450]);
hold on;

% 设定 20m 和 50m 的独立颜色与散点样式以便区分
plot_colors = {'#D95319', '#0072BD'}; % 20m: 红, 50m: 蓝
plot_markers = {'o', 'd'};            % 20m: 圆, 50m: 菱形
...
for d_idx = 1:num_D
    ...
    % 提取单一样本数据
    samples = squeeze(Intensity_Samples(1, 1, d_idx, :));
    mean_I = mean(samples);
    ...
    plot(bin_centers, pdf_counts, m, 'Color', c, ...
         ...
         'DisplayName', sprintf('Sim. %s (L=%dm)', turb_params(1).name, L));
    ...
    if SI > 0
        ...
        plot(I_axis, PDF_theo, '-', 'Color', c, 'LineWidth', 1.5, ...
             'DisplayName', sprintf('Theo. (L=%dm, SI=%.3f)', L, SI));
    end
end
...
title(['Received Intensity PDF: ', water_types{1}], 'FontSize', 14, 'FontName', 'Times New Roman');
