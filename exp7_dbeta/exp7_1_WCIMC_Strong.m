%% Exp7: WCI-MC Pointing Error Simulation (Strong Turbulence Only)
clc; clear; close all;
...
Rx_Aperture = 0.01; Rx_FOV = 90 * pi / 180; Rx_Area = pi * (Rx_Aperture / 2)^2;
...
fprintf('--- 运行 Exp7: WCI-MC (Strong Turbulence) ---\n');
...
for w_idx = 1:num_W
    ...
    for a_idx = 1:num_A
        ...
        for d_idx = 1:num_D
            ...
            [rho0_Link, Cn2] = calc_turb_coherence_params(Phi_func, k_wave, L, eta_physical); delta_z_screen = L / N_screens;
            ...
            for p = 1:N_packets
                ...
                if plane_hit
                    ...
                    if cos_rx_tilt >= cos_FOV_half
                        ...
                        W_spot = W_turb_LT * cf; % [修复]: 解除 W_geo 重复计数
                ...
                for ord = 1:n_max
                    ...
                    % if cos_inc < cos_FOV_half % [修复]: 添加强制FOV截断
                    %     break; 
                    % end
                if cos_inc>=cos_FOV_half
                    ...
                    % weight = weight * max(0.5,min(2, p_val / q_HG)); % [修复]: 权重钳位
                    weight = weight * min(1e3, p_val / q_HG); % [修复]: 权重钳位
...
save('data_exp7_WCIMC_Strong.mat', 'dist_cell', 'PL_Cell_WCI_Strong', 'water_types', 'off_axis_angles');
