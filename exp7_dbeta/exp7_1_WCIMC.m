%% Exp7: WCI-MC Pointing Error Simulation (Strong, Weak, No Turbulence)
% 实验目的: 分析收发偏轴误差 (Off-axis angle) 对链路衰减的影响
% 湍流模式: 强湍流 (Strong), 弱湍流 (Weak), 无湍流 (None)
clc; clear; close all;
cd(fileparts(mfilename('fullpath')));

% ================= 物理参数初始化 =================
% 为保证多维矩阵遍历的仿真时长合理，距离步长设定为 10m
...
for w_idx = 1:num_W
    ...
    for t_idx = 1:3
        ...
        is_turbulent = ~strcmp(tp.name, 'None');
        if is_turbulent
        ...
        for a_idx = 1:num_A
            ...
            % [核心物理变更]: 初始发射基准轴线绕 X 轴旋转 theta_err
            Tx_Dir = [0, cos(theta_err), sin(theta_err)]; 
            
            fprintf('\n[%s] 湍流 | %s | 偏轴: %.1f deg\n', tp.name, water_types{w_idx}, off_axis_angles(a_idx));
            ...
            for d_idx = 1:num_D
                ...
                if is_turbulent
                    ...
                    for i = 1:N_screens
                        ...
                        Grad_X_3D(:, :, i) = gx; Grad_Y_3D(:, :, i) = gy;
                        Screen_Z_1D(i) = i * delta_z_screen; 
                    end
                else
                    rho0_Link = 1e9; % 无湍流相干长度趋于无穷
                    for i = 1:N_screens, Screen_Z_1D(i) = i * delta_z_screen; end
                end
                
                Lx = Tx_Dir(1); Ly = Tx_Dir(2); Lz = Tx_Dir(3);
                Nx = Rx_Normal(1); Ny = Rx_Normal(2); Nz = Rx_Normal(3);
                Tx = Tx_Pos(1); Ty = Tx_Pos(2); Tz = Tx_Pos(3); Rx = Rx_Pos(1); Ry = Rx_Pos(2); Rz = Rx_Pos(3);
                Ux = u_vec_Tx(1); Uy = u_vec_Tx(2); Uz = u_vec_Tx(3); Vx = v_vec_Tx(1); Vy = v_vec_Tx(2); Vz = v_vec_Tx(3);
                ...
                for p = 1:N_packets
                    ...
                    if plane_hit
                        ...
                        if cos_rx_tilt >= cos_FOV_half
                            cx = (p2_end-Ry)*d3_end - (p3_end-Rz)*d2_end;
                            cy = (p3_end-Rz)*d1_end - (p1_end-Rx)*d3_end;
                            cz = (p1_end-Rx)*d2_end - (p2_end-Ry)*d1_end;
                            r_wander_perp = sqrt(cx^2 + cy^2 + cz^2);
                            r_eff = (Rx_Aperture/2) * sqrt(cos_rx_tilt);
                            
                            W_geo = w0 + path_len_ballistic * tan(theta_half_div);
                            if is_turbulent
                                rho0_b = rho0_Link * (L / path_len_ballistic)^(3/5);
                                W_ST = (2*path_len_ballistic/(k_wave*rho0_b)) * max(0, 1 - 0.37*(rho0_b/(2*w0))^(1/3));
                            else
                                W_ST = 0;
                            end
                            % W_spot = sqrt(W_geo^2 + W_ST^2);
                            W_spot = W_ST;
                    ...
                    for ord = 1:n_max
                        ...
                        if cos_inc >= cos_FOV_half
                            ...
                            if base_w > 1e-15
                                ...
                                if v_hit
                                    ...
                                    cx = (p2_v-Ry)*d3_v - (p3_v-Rz)*d2_v; cy = (p3_v-Rz)*d1_v - (p1_v-Rx)*d3_v; cz = (p1_v-Rx)*d2_v - (p2_v-Ry)*d1_v;
                        ...
                        weight = weight * min(1e2, pdf_Empirical(cos_th_i, param) / ((1 - g_prop^2) / (4 * pi * denom_core * sqrt(denom_core))));
        ...
        if t_idx == 1, PL_Cell_WCI_Strong{w_idx} = PL_matrix; elseif t_idx == 2, PL_Cell_WCI_Weak{w_idx} = PL_matrix; else, PL_Cell_WCI_None{w_idx} = PL_matrix; end
    end
end
save('data_exp7_WCIMC.mat', 'dist_cell', 'off_axis_angles', 'PL_Cell_WCI_Strong', 'PL_Cell_WCI_Weak', 'PL_Cell_WCI_None', 'water_types');
fprintf('Exp7 WCI-MC 偏轴数据已保存！\n');

% ================= 辅助计算函数 (保持与 exp2 高度一致) =================
function [p1, p2, p3, d1, d2, d3, hit_flag, total_len] = ray_march_flat_scalar(p1, p2, p3, d1, d2, d3, dist_limit, Rx, Ry, Rz, Rx_Aperture_half_sq, cos_FOV_half, Nx, Ny, Nz, enable_hit_check, Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x0, dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen)
    ...
    while rem_dist > 1e-9
        ...
        if dir_z > 1e-10
            target_idx = floor((z_pos + 1e-9) / delta_z_screen) + 1; if target_idx < 1, target_idx = 1; end; if target_idx <= N_screens, t_scr = (Screen_Z_1D(target_idx) - z_pos) / dir_z; end
        elseif dir_z < -1e-10
            target_idx = ceil((z_pos - 1e-9) / delta_z_screen) - 1; if target_idx > N_screens, target_idx = N_screens; end; if target_idx >= 1, t_scr = (Screen_Z_1D(target_idx) - z_pos) / dir_z; end
        end
        t_rx = inf;
        if enable_hit_check
            denom_rx = d1*Nx + d2*Ny + d3*Nz;
            if abs(denom_rx) > 1e-10, t_temp = ((Rx-p1)*Nx + (Ry-p2)*Ny + (Rz-p3)*Nz) / denom_rx; if t_temp > -1e-7, t_rx = max(0, t_temp); end; end
        end
...
function [Phi_n, k, eta] = get_OTOPS_spectrum_handle(lam, T, S, eps, chi_T, H_r)
    lam_nm=lam*1e9; k=2*pi/lam; A=-1.05e-6*S+2*1.6e-8*T*S-2*2.02e-6*T-4.23e-3/lam_nm; B=1.779e-4-1.05e-6*T+1.6e-8*T^2+1.155e-2/lam_nm; s_f=S*1e-3; T_k=T+273.15;
    ...
    sig_T=10^(log10(240+0.0002*(S/1.00472))-3+0.434*(2.3-(343.5+0.037*(S/1.00472))/(1.00024*T+273.15))*(1-(1.00024*T+273.15)/(647.3+0.03*(S/1.00472)))^(1/3)); 
    Pr=mu*cp/sig_T; Sc=mu^2/(5.954e-15*T_k*rho); c_T=0.072^(4/3)/Pr; c_S=0.072^(4/3)/Sc; c_TS=0.072^(4/3)*(Pr+Sc)/(2*Pr*Sc);
    R_rho=2.6e-4*abs(H_r)/7.6e-4; if R_rho>=1, d_r=R_rho+sqrt(R_rho)*sqrt(R_rho-1); elseif R_rho>=0.5, d_r=1.85*R_rho-0.85; else, d_r=0.15*R_rho; end
    chi_S=chi_T*d_r/(H_r^2); chi_TS=chi_T*(1+d_r)/(2*H_r); eta=(mu/rho)^0.75/eps^0.25; 
    Phi_Hill=@(K,chi_M,c_M) (0.72/(4*pi))*chi_M*eps^(-1/3).*(K.^2+1e-25).^(-11/6).*exp(-176.90*(K*eta).^2.*c_M^(0.96)).*(1+21.61*(K*eta).^(0.61).*c_M^(0.02)-18.18*(K*eta).^(0.55).*c_M^(0.04));
    Phi_n=@(K) (A^2*Phi_Hill(K,chi_T,c_T)+B^2*Phi_Hill(K,chi_S,c_S)+2*A*B*Phi_Hill(K,chi_TS,c_TS));
end
function screen = gen_screen_from_spectrum(Phi_n, D, N, k_wave, dz)
    dk=2*pi/D; dx=D/N; kx=(-N/2:N/2-1)*dk; [KX,KY]=meshgrid(kx,kx); K=sqrt(KX.^2+KY.^2); K(N/2+1,N/2+1)=1e-10;
    C=(randn(N)+1i*randn(N)).*sqrt(2*pi*k_wave^2*dz*Phi_n(K))*dk; screen=real(ifftshift(ifft2(ifftshift(C))))*N^2;
    p_low=zeros(N,N); [xx,yy]=meshgrid((-N/2:N/2-1)*dx);
    for p=1:3, dkp=dk/(3^p); for m=-1:1, for n=-1:1, if m==0&&n==0, continue; end; kp=sqrt((m*dkp)^2+(n*dkp)^2); p_low=p_low+real((randn(1)+1i*randn(1))*sqrt(2*pi^2*k_wave^2*dz*Phi_n(kp))*dkp*exp(1i*(m*dkp*xx+n*dkp*yy))); end; end; end
    screen = screen + p_low;
end
function [rho0, Cn2] = calc_turb_coherence_params(Phi_n, k, L, eta)
    Cn2=Phi_n(0.01/eta)/(0.033*(0.01/eta)^(-11/3));
    try, opts=optimset('Display','off'); rho0=fzero(@(r) 8*pi^2*k^2*L*integral2(@(K,xi) K.*Phi_n(K).*(1-besselj(0,K.*r.*xi)),1e-2,1e5,0,1,'Method','iterated','RelTol',1e-3)-2,(0.545*k^2*Cn2*L)^(-3/5),opts); catch, rho0=(0.545*k^2*Cn2*L)^(-3/5); end
end