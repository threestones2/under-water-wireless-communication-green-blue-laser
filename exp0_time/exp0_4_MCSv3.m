%% 3. MCS (Local Estimation + Inverse Transform Root-Finding)
clc; clear; close all;
...
fprintf('--- 启动 MCS 仿真 (传统物理基准 - fzero非线性寻根法) ---\n');
...
for wt_idx = 1:3
    ...
    for i = 1:num_photons
        ...
        for scnt = 1:max_scattering
            ...
            % 【核心耗时点】：通过 fzero 执行非线性反变换寻根
            xi = rand();
            obj_fun = @(th) interp1(th_axis, cdf_vals, th, 'linear', 'extrap') - xi;
            options = optimset('Display', 'off');
            theta_s = fzero(obj_fun, [0, pi], options);
