%% 3. MCS (Local Estimation + Native Bisection Root-Finding)
clc; clear; close all;
...
fprintf('--- 启动 MCS 仿真 (传统物理基准 - 底层二分寻根法) ---\n');
...
for wt_idx = 1:3
    ...
    for i = 1:num_photons
        ...
        for scnt = 1:max_scattering
            ...
            % =========================================================================
            % 【核心优化】：底层二分寻根法 (Native Bisection Root-Finding)
            % 废弃极为耗时的 fzero 和 interp1，在数学上等价且完全激活 JIT 编译
            % =========================================================================
            xi = rand();
            idx_L = 1; 
            idx_R = 5000;
            ...
            % O(log N) 复杂度的区间夹逼
            while (idx_R - idx_L) > 1
                idx_mid = floor((idx_L + idx_R) / 2);
                if cdf_vals(idx_mid) < xi
                    idx_L = idx_mid;
                else
                    idx_R = idx_mid;
                end
            ...
            % 在极其微小的锁定区间内进行局部线性解析 (相当于牛顿/割线迭代的末端解析)
            cdf_L = cdf_vals(idx_L);
            cdf_R = cdf_vals(idx_R);
            if cdf_R > cdf_L
                theta_s = th_axis(idx_L) + (xi - cdf_L) * (th_axis(idx_R) - th_axis(idx_L)) / (cdf_R - cdf_L);
            else
                theta_s = th_axis(idx_L);
            end
            % =========================================================================
...
function nd = rotate_direction(d, t, p)
    ...
    else, nd = [sin(t)/denom*(d(1)*d(3)*cos(p)-d(2)*sin(p))+d(1)*cos(t), ...
                sin(t)/denom*(d(2)*d(3)*cos(p)+d(1)*sin(p))+d(2)*cos(t), ...
                -sin(t)*cos(p)*denom+d(3)*cos(t)]; end
    nd = nd / norm(nd);
end