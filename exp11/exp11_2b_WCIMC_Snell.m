%% exp11_2b: WCI-MC (Snell-Only Turbulence) — Jerlov IB 距离扫描
%  广义 Snell 定律湍流 WCI-MC：弹道光子 + 散射虚拟光子 + 湍流相位屏偏折
%  仅含相位屏折射 (广义 Snell 定律)，无 Marcum Q 波束扩展统计
%  固定光子数 N=1e5，扫描不同传输距离下的路径衰减
...
fprintf('--- exp11_2b: WCI-MC (Snell-Only) Jerlov IB 距离扫描 N=%d ---\n', N_packets);
fprintf('湍流: %s (仅相位屏偏折, 无 Marcum Q) | D_screen=%.1fm | N_grid=%d | dz~%.0fm\n\n', ...
...
for d_idx = 1:num_D
    ...
    fprintf('  [屏] L=%2dm -> N_screens=%d, dz=%.1fm, dx=%.2fmm\n', ...
    ...
    fprintf('  %s | L=%2dm | N=%d | Time=%5.2fs | PL=%6.2f dB\n', ...
        water_type, L, N_packets, Time_arr(d_idx), -PL_arr(d_idx));
end

save('data_exp11_WCIMC_Snell.mat', 'dist_arr', 'PL_arr', 'Time_arr', 'N_packets', 'water_type');
fprintf('已保存至 data_exp11_WCIMC_Snell.mat\n');
