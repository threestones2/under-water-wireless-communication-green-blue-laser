%% exp11_5: MCS Physical (Snell-Only Turbulence) — Jerlov IB 距离扫描
%  纯物理接收 MCS + 相位屏偏折 (广义 Snell 定律)：光子必须几何命中接收孔径
%  无半解析近似、无 Marcum Q 波束扩展，仅含相位屏折射
%  每个距离对 N_turb 个独立湍流相位屏实现取平均，消除单次随机扰动偏差
%  光子数 N=1e5，扫描不同传输距离下的路径衰减
...
turb_params.name = 'Physical+Snell';
...
PL_arr = zeros(1, num_D);       % 平均路径损耗 (dB)
PL_std_arr = zeros(1, num_D);   % 标准差 (dB)
Time_arr = zeros(1, num_D);
PL_all = zeros(num_D, N_turb);  % 每个距离每帧实现余量, 用于调试/画误差棒

fprintf('--- exp11_5: MCS Physical (Snell-Only) Jerlov IB 距离扫描 ---\n');
fprintf('N=%d/帧 | N_turb=%d (相位屏实现) | 总光子/距离=%d\n', ...
    N_packets, N_turb, N_packets * N_turb);
fprintf('湍流: %s (纯物理接收 + 相位屏偏折, 无 Marcum Q) | D_screen=%.1fm | N_grid=%d\n\n', ...
    turb_params.name, D_screen, N_grid);

[Phi_func, ~, eta_physical] = get_OTOPS_spectrum_handle(lambda, ...
    turb_params.T_avg, turb_params.S_avg, turb_params.epsilon, ...
    turb_params.chi_T, turb_params.H_ratio);
...
for d_idx = 1:num_D
    ...
    % 自适应相位屏
    N_screens = max(3, ceil(L / delta_z_target));
    delta_z_screen = L / N_screens;

    fprintf('  [L=%2dm] N_screens=%d, dz=%.1fm, dx=%.2fmm\n', ...
        L, N_screens, delta_z_screen, dx*1e3);

    x_axis = (-N_grid/2 : N_grid/2-1) * dx;

    t_start = tic;
    P_linear_all = zeros(1, N_turb);

    % ===== 湍流相位屏实现循环 =====
    for t_idx = 1:N_turb
        % 每帧生成独立的相位屏 (gen_screen_from_spectrum 内部使用 randn)
        ...
        % 光子循环 (固定种子保证不同帧间光子抽样可对比)
        P_rx_accum = 0;
        rng(123456 + t_idx - 1, 'twister');
        ...
        for p = 1:N_packets
            ...
            % --- 弹道检查 (含相位屏偏折, 纯几何命中) ---
            [p1_bal, p2_bal, p3_bal, d1_bal, d2_bal, d3_bal, bal_hit, bal_len] = ...
                ray_march_flat_turb(p1, p2, p3, d1, d2, d3, 1e9, ...
                Rx, Ry, Rz, 1e9, -1, Nx, Ny, Nz, true, ...
                Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, ...
                k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);

            if bal_hit
                cos_rx_tilt = abs(d1_bal*Nx + d2_bal*Ny + d3_bal*Nz);
                if cos_rx_tilt >= cos_FOV_half
                    vec_x = p1_bal - Rx;  vec_y = p2_bal - Ry;  vec_z = p3_bal - Rz;
                    cx = vec_y*d3_bal - vec_z*d2_bal;
                    cy = vec_z*d1_bal - vec_x*d3_bal;
                    cz = vec_x*d2_bal - vec_y*d1_bal;
                    r_wander_perp = sqrt(cx^2 + cy^2 + cz^2);
                    r_eff = (Rx_Aperture/2) * sqrt(cos_rx_tilt);
                    if r_wander_perp <= r_eff
                        P_rx_accum = P_rx_accum + exp(-param.coef_c * bal_len);
                        continue;
                    end
                end
            end

            % --- 散射游走 (含相位屏偏折, 每步 ray_march) ---
            for ord = 1:n_max
                d_step = -log(rand()) / param.coef_b;

                [p1, p2, p3, d1, d2, d3, rx_hit, step_len] = ...
                    ray_march_flat_turb(p1, p2, p3, d1, d2, d3, d_step, ...
                    Rx, Ry, Rz, Rx_Aperture_half_sq, cos_FOV_half, Nx, Ny, Nz, true, ...
                    Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, ...
                    k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);

                if rx_hit
                    P_packet = P_packet + weight * exp(-param.coef_c * step_len);
                    break;
                end
        ...
        P_linear = max(P_rx_accum / N_packets, 1e-300);
        P_linear_all(t_idx) = P_linear;
        PL_all(d_idx, t_idx) = 10 * log10(P_linear);
        fprintf('  L=%2dm | 帧 %2d/%2d | PL=%6.2f dB\n', ...
            L, t_idx, N_turb, -PL_all(d_idx, t_idx));
    end

    % 线性域平均 → dB
    P_avg = mean(P_linear_all);
    PL_arr(d_idx) = 10 * log10(max(P_avg, 1e-300));
    PL_std_arr(d_idx) = std(PL_all(d_idx, :));
    Time_arr(d_idx) = toc(t_start);
    fprintf('  => L=%2dm | Avg PL=%6.2f dB | Std=%5.2f dB | Time=%5.1fs\n\n', ...
        L, -PL_arr(d_idx), PL_std_arr(d_idx), Time_arr(d_idx));
end

save('data_exp11_MCS_Physical_Turb.mat', ...
    'dist_arr', 'PL_arr', 'PL_std_arr', 'PL_all', 'Time_arr', ...
    'N_packets', 'N_turb', 'water_type');
fprintf('已保存至 data_exp11_MCS_Physical_Turb.mat (含 PL_arr, PL_std_arr, PL_all)\n');

% ================= 湍流光线下辅助函数 =================
function [p1, p2, p3, d1, d2, d3, hit_flag, total_len] = ray_march_flat_turb(p1, p2, p3, d1, d2, d3, dist_limit, ...
        Rx, Ry, Rz, Rx_Aperture_half_sq, cos_FOV_half, Nx, Ny, Nz, enable_hit_check, ...
        Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x0, dx, N_grid, ...
    ...
    for p = 1:3
        ...
        for m = -1:1
            for n = -1:1
                ...
                phase_low = phase_low + real((randn(1)+1i*randn(1)) * sqrt(2*pi^2*k_wave^2*delta_z*Phi_n_func(k_p)) * dk_p * exp(1i*(m*dk_p*xx + n*dk_p*yy)));
            end
        end
    end
