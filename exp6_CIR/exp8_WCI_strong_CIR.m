%% Exp8: WCI-MC CIR Simulation 
% 实验目的: 使用WCI-MC算法计算信道冲激响应(CIR)，采用Haltrin相函数与PIS重要性采样
clc; clear; close all;
...
CIR_WCIMC_Scattered = cell(1, num_water);
...
for w = 1:num_water
    ...
    fprintf('\n======================================================\n');
    fprintf('>>> WCI-MC CIR 仿真 | 纯Haltrin拟合 + PIS | %s <<<\n', wp.name);
    ...
    for p = 1:N_packets
        ...
        for ord = 1:n_max
            ...
            w_mcs = w_mcs * min(1e3, p_val / q_HG);
            dir_new = rotate_direction_fast([d1, d2, d3], cos_th_i, sqrt(1 - cos_th_i^2), 2*pi*rand()); 
            d1 = dir_new(1); d2 = dir_new(2); d3 = dir_new(3);
        end
    end
    CIR_WCIMC_Scattered{w} = h_time_WCIMC / N_packets;
    E_Ballistic_Total{w} = E_ballistic_accum / N_packets;
    fprintf('  计算完成, 耗时: %5.2f s\n', toc);
end

save('data_exp8_CIR_WCIMC.mat', 'water_params', 'Time_Axis', 'CIR_WCIMC_Scattered', 'E_Ballistic_Total', 'dt');
disp('WCI-MC CIR 数据已保存！');
