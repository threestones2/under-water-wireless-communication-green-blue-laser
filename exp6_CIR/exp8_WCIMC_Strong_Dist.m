%% Exp8: WCI-MC CIR Simulation (多距离遍历版)
% 实验目的: 计算不同水质、不同传输距离下的信道冲激响应(CIR)
clc; clear; close all;
cd(fileparts(mfilename('fullpath')));

N_packets = 1e6;  % 建议在实际跑图时调大至 1e7 以保证远距离尾部平滑
n_max = 200; 
c_water = 2.237e8;
lambda = 514e-9; 
k_wave = 2 * pi / lambda;
...
turb_params.epsilon = 1e-10; turb_params.chi_T = 1e-5; 
...
% --- 水质与距离参数设定 ---
water_params = struct();
water_params(1).name = 'Clear Ocean';   water_params(1).a = 0.114; water_params(1).b = 0.0374; water_params(1).c = 0.1514; water_params(1).L_arr = 10:10:50;
water_params(2).name = 'Coastal Ocean'; water_params(2).a = 0.179; water_params(2).b = 0.219;  water_params(2).c = 0.398;  water_params(2).L_arr = 5:5:20;
...
% 数据存储结构: {水质索引, 距离索引}
CIR_WCIMC_Scattered = cell(num_water, 10);
E_Ballistic_Total = cell(num_water, 10); 
Time_Axis = cell(num_water, 10);
...
for w = 1:num_water
    ...
    for d_idx = 1:length(wp.L_arr)
        L = wp.L_arr(d_idx);
        t_LOS = L / c_water; t_min = t_LOS; t_max = t_LOS + delta_t_max;
        T_bins = t_min : dt : t_max; N_bins = length(T_bins); 
        Time_Axis{w, d_idx} = T_bins - t_LOS; 
        
        h_time_WCIMC = zeros(1, N_bins); E_ballistic_accum = 0;         
        
        fprintf('  水质: %s | 距离: %2d m | 计算中...\n', wp.name, L);
        ...
        for i = 1:N_screens
            ...
            [gx, gy] = gradient(phi_screen, dx); Grad_X_3D(:,:,i) = gx; Grad_Y_3D(:,:,i) = gy; Screen_Z_1D(i) = i * delta_z_screen; 
        end
        ...
        for p = 1:N_packets
            ...
            [p1_end, p2_end, p3_end, d1_end, d2_end, d3_end, plane_hit, path_len_ballistic] = ...
                ray_march_flat_scalar(p1, p2, p3, d1, d2, d3, 1e9, Rx, Ry, Rz, 1e10, -1, Nx, Ny, Nz, true, ...
                Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);
            ...
            if plane_hit
                ...
                if cos_rx_tilt >= cos_FOV_half
                    ...
                    E_ballistic_accum = E_ballistic_accum + exp(-wp.c * path_len_ballistic) * recv_frac;
                end
            end
            ...
            for ord = 1:n_max
                ...
                [p1, p2, p3, d1, d2, d3, ~, step_len] = ray_march_flat_scalar(p1, p2, p3, d1, d2, d3, d_step, Rx, Ry, Rz, Rx_Aperture_half_sq, cos_FOV_half, Nx, Ny, Nz, false, ...
                    Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);
                
                w_mcs = w_mcs * exp(-wp.a * step_len); 
                ...
                if cos_inc >= cos_FOV_half && dist_total > (L + 1e-9)
                    ...
                    base_w = w_mcs * min(1, pdf_Empirical(cos_scatter, param) * omega) * exp(-wp.c * dist2rx);
                    
                    if base_w > 1e-16
                        [p1_v, p2_v, p3_v, d1_v, d2_v, d3_v, v_hit, ~] = ray_march_flat_scalar(p1, p2, p3, dir2rx_1, dir2rx_2, dir2rx_3, dist2rx+1e-1, Rx, Ry, Rz, 1e10, -1, Nx, Ny, Nz, true, ...
                            Grad_X_3D, Grad_Y_3D, Screen_Z_1D, Ux, Uy, Uz, Vx, Vy, Vz, k_wave, x_axis(1), dx, N_grid, Tx, Ty, Tz, Lx, Ly, Lz, delta_z_screen);
                        
                        if v_hit
                            cos_inc_v = abs(d1_v*Nx + d2_v*Ny + d3_v*Nz); r_eff_v = r_rx * sqrt(cos_inc_v);
                            ...
                            if idx >= 1 && idx <= N_bins
                                h_time_WCIMC(idx) = h_time_WCIMC(idx) + base_w * point_loss; 
                            end
                        end
                    end
                end
                ...
                w_mcs = w_mcs * min(1e3, pdf_Empirical(cos_th_i, param) / q_HG);
                dir_new = rotate_direction_fast([d1, d2, d3], cos_th_i, sqrt(1 - cos_th_i^2), 2*pi*rand()); 
                d1 = dir_new(1); d2 = dir_new(2); d3 = dir_new(3);
            end
        end
        CIR_WCIMC_Scattered{w, d_idx} = h_time_WCIMC / N_packets;
        E_Ballistic_Total{w, d_idx} = E_ballistic_accum / N_packets;
        fprintf('    耗时: %5.2f s\n', toc);
    end
end
save('data_exp8_CIR_Dist.mat', 'water_params', 'Time_Axis', 'CIR_WCIMC_Scattered', 'E_Ballistic_Total', 'dt');
disp('多距离数据已保存为 data_exp8_CIR_Dist.mat');

% --- 辅助函数 ---
function [p1,p2,p3,d1,d2,d3,hit_flag,total_len]=ray_march_flat_scalar(p1,p2,p3,d1,d2,d3,dist_limit,Rx,Ry,Rz,Rx_Aperture_half_sq,cos_FOV_half,Nx,Ny,Nz,enable_hit_check,Grad_X_3D,Grad_Y_3D,Screen_Z_1D,Ux,Uy,Uz,Vx,Vy,Vz,k_wave,x0,dx,N_grid,Tx,Ty,Tz,Lx,Ly,Lz,delta_z_screen), hit_flag=false; total_len=0; rem_dist=dist_limit; N_screens=length(Screen_Z_1D); while rem_dist>1e-9, dir_z=d1*Lx+d2*Ly+d3*Lz; z_pos=(p1-Tx)*Lx+(p2-Ty)*Ly+(p3-Tz)*Lz; t_scr=inf; target_idx=-1; if dir_z>1e-10, target_idx=floor((z_pos+1e-9)/delta_z_screen)+1; if target_idx<1, target_idx=1; end; if target_idx<=N_screens, t_scr=(Screen_Z_1D(target_idx)-z_pos)/dir_z; end, elseif dir_z<-1e-10, target_idx=ceil((z_pos-1e-9)/delta_z_screen)-1; if target_idx>N_screens, target_idx=N_screens; end; if target_idx>=1, t_scr=(Screen_Z_1D(target_idx)-z_pos)/dir_z; end; end, t_rx=inf; if enable_hit_check, denom_rx=d1*Nx+d2*Ny+d3*Nz; if abs(denom_rx)>1e-10, t_temp=((Rx-p1)*Nx+(Ry-p2)*Ny+(Rz-p3)*Nz)/denom_rx; if t_temp>-1e-7, t_rx=max(0,t_temp); end, end, end, [min_dist,event_idx]=min([rem_dist,t_rx,t_scr]); p1=p1+d1*min_dist; p2=p2+d2*min_dist; p3=p3+d3*min_dist; rem_dist=rem_dist-min_dist; total_len=total_len+min_dist; if event_idx==2, if (p1-Rx)^2+(p2-Ry)^2+(p3-Rz)^2<=Rx_Aperture_half_sq && -(d1*Nx+d2*Ny+d3*Nz)>=cos_FOV_half, hit_flag=true; end, break; elseif event_idx==3, loc_u=(p1-Tx)*Ux+(p2-Ty)*Uy+(p3-Tz)*Uz; loc_v=(p1-Tx)*Vx+(p2-Ty)*Vy+(p3-Tz)*Vz; idx_x=mod(round((loc_u-x0)/dx),N_grid)+1; idx_y=mod(round((loc_v-x0)/dx),N_grid)+1; gx=Grad_X_3D(idx_y,idx_x,target_idx); gy=Grad_Y_3D(idx_y,idx_x,target_idx); d1=d1+(gx*Ux+gy*Vx)/k_wave; d2=d2+(gx*Uy+gy*Vy)/k_wave; d3=d3+(gx*Uz+gy*Vz)/k_wave; d_norm=sqrt(d1^2+d2^2+d3^2); d1=d1/d_norm; d2=d2/d_norm; d3=d3/d_norm; elseif event_idx==1, break; end, end, end
