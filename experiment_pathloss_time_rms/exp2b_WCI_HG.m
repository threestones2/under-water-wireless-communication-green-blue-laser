%% 2. Adaptive HG-PIS vs Distance (Without Turbulence)
clc; clear; close all;
...
p_peak = pdf_Empirical(1.0, param); C_val = 4 * pi * p_peak;
g_prop = 1 + (1 - sqrt(8*C_val + 1)) / (2*C_val);

fprintf('=== 启动 Adaptive HG-PIS 距离演化仿真 ===\n');
...
for d_idx = 1:length(dist_array)
    ...
    for p = 1:N_packets
        ...
        if cos_theta > 0
            ...
            pos_end = Tx_Pos + dir * dist_to_plane;
        ...
        for order = 1 : n_max
            ...
            if acos(dot(-dir2rx, Rx_Normal)) <= Rx_FOV/2
                w_rx = weight * param.albedo * min(1, pdf_Empirical(dot(dir, dir2rx), param) * (Rx_Area / dist2rx^2 * abs(dot(dir2rx, Rx_Normal)))) * exp(-param.coef_c * dist2rx);
            ...
            xi = rand(); term = (1 - g_prop^2) / (1 - g_prop + 2 * g_prop * xi); cos_th_i = max(min((1 + g_prop^2 - term^2) / (2 * g_prop), 1), -1);
            weight = weight * min(1e3, pdf_Empirical(cos_th_i, param) / ((1 - g_prop^2) / (4 * pi * (1 + g_prop^2 - 2 * g_prop * cos_th_i)^1.5)));
            dir = rotate_direction(dir, acos(cos_th_i), 2*pi*rand());
        end
    end
    t_run = toc;
