...
% 本代码是将AI的debug结果拿来验证的代码，可随意乱动，复制于MCI_PIS
...
function [P, P_hat, h_hat, h_hat_i] = MCI_PIS_Function(param)
    ...
    ke = ka + ks;
    % 显示中间结果
    disp(['散射系数 ks = ', num2str(ks)]);
    disp(['消光系数 ke = ', num2str(ke)]);
    disp(['ks/ke 比值 = ', num2str(ks/ke)]);
    % 计算方向余弦
    ...
    for k = 1:N
        ...
        for i = 1:n_max
            ...
            % 计算目标函数的乘积因子
            if i == 1
                f_Theta = sin(theta_i) / (1 - cos(beta_T/2));  % 初始发射光束的PDF
            ...
            if cos_phi_r >= cos(beta_R/2)
                ...
                % 使用正确的检测概率计算公式（添加min函数限制）
                p_d = ks/ke * exp(-ke * d_to_rx)^2 * cos_phi_r * min(1, f_Theta_n * Omega_r / (2*pi*sin(theta_n)));
                %p_d = exp(-ke * d_to_rx) * cos_phi_r * min(1, f_Theta_n * Omega_r / (2*pi*sin(theta_n)));
